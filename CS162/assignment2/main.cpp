...
int main() {
	...
	num_employees= employee_file();
	employees_ (employees,num_employees,read_employee);
...
int employee_file(){
	...
	string line;
	ifstream read_employee;
	read_employee.open("employee.txt");
		if(!read_employee.is_open()){
			cout << "error" << endl;
			exit(1);
		}
	while(getline(read_employee, line))
		++num_employees;//checking number of lines in the file
