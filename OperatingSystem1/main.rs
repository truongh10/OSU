...
fn main() {
    ...
    for i in 0..num_partitions {
       let part = xs[i].clone();
       thread.push(thread::spawn(move || map_data(&part)));
    }
    // 4. Collects the intermediate sums from all the threads
    let mut intermediate_sums:Vec<usize> = Vec::new();
    for i in 0..num_partitions {
        let t = thread.remove(0);
    ...
    let tsum = reduce_data(&intermediate_sums);
...
//    partition_data_in_two(v)
    let mut remain = v.len() % num_partitions;
